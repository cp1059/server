...
def customFuncForCp(**kwargs):
    ...
    html = request(method="GET",url="https://www.52cp.cn/fc3d/history.html",headers=headers,timeout=5).text
    pattern = re.compile(r"openInfo?.*?};")

    res = json.loads((pattern.findall(html)[0].split(';')[0].split('=')[1]))
